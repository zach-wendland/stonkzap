"""
Database layer with support for both PostgreSQL and in-memory storage.

By default, uses in-memory storage (no Docker required).
...
class DB:
    ...
    - In-memory storage (default, no external dependencies)
    ...
    def upsert_post(self, p: SocialPost) -> int:
        ...
        if self.backend == 'memory':
            return self.storage.upsert_post(p)
        else:
            with self.conn.cursor() as c:
                c.execute("""
    ...
    def upsert_sentiment(self, pk: int, s: SentimentScore) -> None:
        """Insert or update sentiment score."""
        if self.backend == 'memory':
            self.storage.upsert_sentiment(pk, s)
        else:
            with self.conn.cursor() as c:
                c.execute("""
    ...
    def upsert_embedding(self, pk: int, emb: np.ndarray) -> None:
        ...
        if self.backend == 'memory':
            self.storage.upsert_embedding(pk, emb.tobytes())
        else:
            with self.conn.cursor() as c:
                c.execute("""
    ...
    def aggregate(self, symbol: str, since: datetime) -> Dict:
        ...
        if self.backend == 'memory':
            return self.storage.aggregate(symbol, since)
        else:
            with self.conn.cursor() as c:
                c.execute("""
                    SELECT
    ...
    def cache_resolution(self, query: str, symbol: str, cik: Optional[str],
        ...
        if self.backend == 'memory':
            self.storage.cache_resolution(query, symbol, cik, isin, figi, company_name)
        else:
            with self.conn.cursor() as c:
                c.execute("""
    ...
    def get_cached_resolution(self, query: str) -> Optional[Dict]:
        ...
        if self.backend == 'memory':
            return self.storage.get_cached_resolution(query)
        else:
            with self.conn.cursor() as c:
                c.execute("""
    ...
    def get_stats(self) -> Dict:
        ...
        if self.backend == 'memory':
            ...
            # Could implement PostgreSQL stats if needed
